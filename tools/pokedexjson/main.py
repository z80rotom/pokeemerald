...
GBASE_STATS_SCAN = "const struct PokedexEntry gPokedexEntries[] ="
SPECIES_NAME_SCAN = "["
REGEX_SPECIES = r".*?\[(.*)].*"
SPECIES_START_SCAN = "{"
SPECIES_END_SCAN = "}"
...
def category_name_parser(field, value, species_data):
    field = to_snake_case(field)
    value = value[value.find('_("') + 3 : value.find('")')]
    species_data[field] = value

FIELD_PARSERS = {
    "categoryName" : category_name_parser,
    "height" : int_field_parser,
    "weight" : int_field_parser,
    "description" : basic_field_parser,
    "pokemonScale" : int_field_parser,
    "pokemonOffset" : int_field_parser,
    "trainerScale" : int_field_parser,
    "trainerOffset" : int_field_parser,
}
...
def convert_to_json(fobj):
    ...
    for i, line in enumerate(fdata.split('\n')):
        ...
        if line.startswith(SPECIES_NAME_SCAN):
            ...
            raw_name = match[0]
            # Remove leading NATIONAL_DEX_ and lower for simplicity
            last_species_name = current_species_name
            last_species_data = current_species_data
            current_species_name = '_'.join(raw_name.split("_")[2:]).lower()
